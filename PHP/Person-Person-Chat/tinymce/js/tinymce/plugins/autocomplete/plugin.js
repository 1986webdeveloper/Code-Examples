	...
	tinymce.create('tinymce.plugins.AutoCompletePlugin', {
		...
		init: function (ed, url) {
			...
			function displayOptionList(matches, matchedText, ed) {
				...
				for (var i in matches) {
					if (matches[i].key != null) {
						matchesList += "<li class='myauto' data-value='" + matches[i].key + "'>" + matches[i].key.replace(highlightRegex, "<mark>$1</mark>") + " " + matches[i].description + "</li>";
					}
					else {
						matchesList += "<li class='myauto' data-value='" + matches[i] + "'>" + matches[i].replace(highlightRegex, "<mark>$1</mark>") + "</li>";
					}
				}
			...
			function hideOptionList() {
				if( ($("ul.auto-list").has("li").length === 0) ) {
				  jQuery(autocomplete_data.list).css("display", "none");

				} else {
					$( ".auto-list .myauto" ).on( "click", function() {
		      			$("ul.auto-list").empty();
		      			jQuery(autocomplete_data.list).css("display", "none");
		      		});
				  jQuery(autocomplete_data.list).css("display", "block");
