	...
	tinymce.create('tinymce.plugins.AutoCompletePlugin', {
		...
		init: function (ed, url) {
			...
			function displayOptionList(matches, matchedText, ed) {
				...
				$(autocomplete_data.list).html(matchesList);
				...
				$(autocomplete_data.list).css("margin-top", tinymcePosition.top + toolbarPosition.innerHeight() + textareaTop);
				$(autocomplete_data.list).css("margin-left", tinymcePosition.left + textareaLeft);
				$(autocomplete_data.list).css("display", "block");
			...
			function optionListEventHandlers(ed) {
				$(autocomplete_data.list).find("li").hover(function () {
					$(autocomplete_data.list).find("[data-selected=true]").attr("data-selected", "false");
					$(this).attr("data-selected", "true");
				});
				$(autocomplete_data.list).find("li").click(function () {
					selectOption(ed, getCurrentWord(ed));
				});
			}
			...
			function createOptionList() {
				...
				$(ulContainer).addClass("auto-list");
			...
			function selectOption(ed, matchedText) {
				var current = $(autocomplete_data.list).find("[data-selected=true]").attr("data-value");
				if (current == null) {
					current = $(autocomplete_data.list).find("li:first-child").attr("data-value");
				}
